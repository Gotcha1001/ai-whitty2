  ...
  try {
    ...
    if (!isSingleRecipe && recipes.length > 7) {
      ...
      for (let j = recipes.length; j < 7; j++) {
        recipes.push({
          name: `Placeholder Fast Food ${j + 1}`,
...
export async function GET() {
  try {
    ...
    const prompt = `${systemMessage}\n\nProvide seven unique fast food recipes, one for each day from Monday to Sunday. Format each recipe with a header like '**Monday: Recipe Name**', followed by Ingredients and Instructions. Ensure each is quick to prepare and includes both ingredients and instructions.`;

    const result = await model.generateContent(prompt);
    const response = await result.response;
    const text = response.text();
    console.log("Raw API response for fast food:", text);
    ...
    if (!recipes || recipes.length === 0) {
      throw new Error("No fast food recipes generated");
    }

    return NextResponse.json({ quirkyResponse, recipes });
  } catch (error) {
    console.error("Error in fast food API:", error);
    return NextResponse.json(
      {
        error:
          error.message ||
          "Chef Quirky couldn't prepare your fast food. Try again!",
      },
      { status: 500 }
    );
  }
