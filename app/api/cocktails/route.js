    ...
    while (i < lines.length) {
      ...
      if (line.startsWith("**") && line.endsWith("**")) {
        ...
        if (isSingleRecipe) {
          ...
          if (header.toLowerCase().includes("weekly")) {
            i++;
            continue;
          }
          for (const d of days) {
          ...
          if (!dayAssigned) {
            if (currentRecipe && currentRecipe.instructions.length) {
              recipes.push(currentRecipe);
            }
            currentRecipe = {
              name: header,
        ...
        if (line.toLowerCase().startsWith("ingredients:")) {
          ...
          if (
            ...
            line.startsWith("• ") ||
            line.match(/^\* \*\*[^\*]+\*\*:/) ||
            line.match(/^\d+\.\s*\d*\.?\d*\s*(oz|ml|g|tbsp|tsp|cup|dash)\s/i) // Matches numbered ingredients like "1.5 oz Gin"
          ) {
            let ingredient = line.replace(/^\* \*\*[^\*]+\*\*:/, "").trim();
            ingredient = ingredient.replace(/^[-*•]\s*|^(\d+\.\s*)/, "").trim();
          ...
          if (
            ...
            if (
              instruction &&
              !instruction.match(/^\d*\.?\d*\s*(oz|ml|g|tbsp|tsp|cup|dash)\s/i)
            ) {
...
export async function GET() {
  try {
    ...
    const prompt = `${systemMessage}\n\nProvide seven unique cocktail recipes, one for each day from Monday to Sunday. Format each recipe with a header like '**Monday: Recipe Name**', followed by Ingredients (using '- ' or numbered items like '1.5 oz Gin') and Instructions (using numbered steps '1. '). Ensure each is creative and includes both ingredients and instructions.`;

    const result = await model.generateContent(prompt);
    const response = await result.response;
    const text = response.text();
    console.log("Raw API response for cocktails:", text);
    ...
    if (!recipes || recipes.length === 0) {
      throw new Error("No cocktail recipes generated");
    }

    return NextResponse.json({ quirkyResponse, recipes });
  } catch (error) {
    console.error("Error in cocktails API:", error);
    return NextResponse.json(
      {
        error:
          error.message ||
          "Chef Quirky couldn't mix your cocktails. Try again!",
      },
      { status: 500 }
    );
  }
