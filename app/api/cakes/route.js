...
export async function GET() {
  try {
    ...
    const prompt = `${systemMessage}\n\nProvide seven delicious cake recipes, one for each day from Monday to Sunday. Format each recipe with a header like '**Monday: Recipe Name**', followed by Ingredients and Instructions. Make them unique and interesting!`;

    const result = await model.generateContent(prompt);
    const response = await result.response;
    const text = response.text();
    console.log("Raw API response for cakes:", text);
    ...
    if (!recipes || recipes.length === 0) {
      throw new Error("No cake recipes generated");
    }

    return NextResponse.json({ quirkyResponse, recipes });
  } catch (error) {
    console.error("Error in cakes API:", error);
    return NextResponse.json(
      {
        error:
          error.message || "Chef Quirky couldn't bake your cakes. Try again!",
      },
      { status: 500 }
    );
  }
